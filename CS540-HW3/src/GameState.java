...
public class GameState {
	...
	public List<GameState> getNextStates() {
		...
		successors.addAll(getNewOneByOnePositions(emptyBlock1X, emptyBlock1Y,
				emptyBlock2X, emptyBlock2Y));
	 ...
	 */
	private List<GameState> getNewOneByOnePositions(int emptyBlock1X,
			int emptyBlock1Y, int emptyBlock2X, int emptyBlock2Y) {
			...
			for (int j = 0; j < 4; j++) {
				if (board[i][j] == 3) {
					...
					if (j < 2 && board[i][j + 2] == 0) {
						...
						board[i][j + 2] = 0;
						board[i][j] = 3;
						break outerloop_1; // if slide right works, slide up or
			...
			for (int j = 0; j < 4; j++) {
				if (board[i][j] == 2 && !skipBlocks.contains(4 * i + j)) {
					...
					if (i > 0 && board[i - 1][j] == 0) {
						...
						board[i - 1][j] = 0;
						board[i + 1][j] = 2;
						continue; // block cannot move left or right if it can
									// move up.
					}
